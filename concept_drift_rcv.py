...
def fresh_model(d):
    return models.LogisticRegression(numpy.random.rand(d), OPT)
    
def carryover_model(model):
    return models.LogisticRegression(numpy.copy(model.w), OPT)
    
def swap_labels(data):
    return [(i, x, -1*y) for (i, x, y) in data]
...
def process(b, rate):
    ...
    train_set, test_set, d = datasets.rcv()
